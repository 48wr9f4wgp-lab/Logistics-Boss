...
func _verify_rank2_actions() -> void:
    ...
    for zone_key in ["storage", "packing"]:
        ...
        for button in zone._rank2_capital_buttons:
            zone.open_zone(zone_key)
            await _settle()
            zone._scroll.ensure_control_visible(button)
            await _settle()
    ...
    _tap(zone._staffing_open)
    await _settle()
    _expect(clarity._section == "staffing", "Zone routes to the shared staffing workspace")
    _tap(clarity.staffing.cards["inbound"])
    await _settle()
    _tap(clarity.staffing.cards["shipping"])
    await _settle()
    _expect(clarity.router.button_at(clarity.staffing.apply_button.get_global_rect().get_center()) == clarity.staffing.apply_button, "Staffing apply stays visible outside the scroll area")
    await _settle()
    _tap(clarity.staffing.apply_button)
    await _settle()
