...
func _init() -> void:
    var rank1: WorkloadWarehouseSim = WorkloadSimScript.new()
    assert(not bool(rank1.workload_wave().get("enabled", true)), "workload waves must remain locked at Rank 1")

    var sim: WorkloadWarehouseSim = _rank2_sim(0.0, "balanced")
    ...
    var normal_inbound: float = sim._current_inbound_interval()
    sim.step(35.1)
    ...
    var order_sim: WorkloadWarehouseSim = _rank2_sim(120.1, "picking")
    ...
    var dispatch_sim: WorkloadWarehouseSim = _rank2_sim(205.1, "shipping")
    ...
    var before_shipped: int = dispatch_sim.shipped
    for _i in 120:
    ...
    var saved: Dictionary = order_sim.save_data()
    assert(int(saved.get("schema_version", -1)) == 4, "workload-aware runtime must save schema 4")
    var restored: WorkloadWarehouseSim = WorkloadSimScript.new()
    ...
    var schema3: Dictionary = saved.duplicate(true)
    schema3["schema_version"] = 3
    schema3.erase("workload_clock")
    schema3.erase("dispatch_start_shipped")
    var migrated: WorkloadWarehouseSim = WorkloadSimScript.new()
...
func _rank2_sim(clock: float, staffing: String) -> WorkloadWarehouseSim:
